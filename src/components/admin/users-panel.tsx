  ...
  return (
    <section>
      <PageHeading
        ...
        }}>{exporting ? "Exporting…" : "Export Excel"}</Button><Link href="/dashboard/user/add" className="rounded-xl bg-cyan-300 px-5 py-3 text-sm font-black text-slate-950">Create user</Link></>}
      />
      ...
      <Panel>
        ...
        {list.isLoading ? <LoadingRows /> : list.error ? <ErrorNotice message={getApiErrorMessage(list.error)} /> : users.length ? (
          <div className="overflow-x-auto"><table className="w-full min-w-[920px] text-left text-sm"><thead className="text-[10px] uppercase tracking-wider text-slate-500"><tr><th className="pb-3">Email</th><th className="pb-3">Role</th><th className="pb-3">Verification</th><th className="pb-3">Lock</th><th className="pb-3">Created</th><th className="pb-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-white/8">{users.map((user) => <tr key={idOf(user)}><td className="py-4"><Link href={`/dashboard/user/${idOf(user)}`} className="font-bold hover:text-cyan-300">{user.email}</Link></td><td className="py-4"><StatusBadge value={user.role} /></td><td className="py-4"><StatusBadge value={user.isVerified ? "Verified" : "Unverified"} /></td><td className="py-4"><StatusBadge value={user.lockedUntil && new Date(user.lockedUntil) > new Date() ? "Locked" : "Open"} /></td><td className="py-4 text-xs text-slate-500">{formatDate(user.createdAt)}</td><td className="py-4 text-right"><Link href={`/dashboard/user/${idOf(user)}`} className="rounded-xl border border-cyan-300/20 bg-cyan-300/[0.06] px-4 py-2.5 text-xs font-bold text-cyan-200">Manage</Link></td></tr>)}</tbody></table></div>
...
export function UserAdminDetail({ userId }: { userId: string }) {
  ...
  async function remove() { if (!user || !window.confirm(`Delete ${user.email} and dependent profile/application data?`)) return; setError(""); try { await deleteUser(userId).unwrap(); router.replace("/dashboard/user"); } catch (caught) { setError(getApiErrorMessage(caught)); } }
