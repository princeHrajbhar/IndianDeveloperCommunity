...
export function FilesPanel() {
  ...
  return <section className="space-y-6">
    ...
    <Panel><PanelTitle eyebrow="Registry" title="Managed assets" /><div className="mb-5 grid gap-3 md:grid-cols-[1fr_220px_auto]"><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} className={inputClass} placeholder="Search label, filename, or storage key" /><select value={providerFilter} onChange={(event) => { setProviderFilter(event.target.value); setPage(1); }} className={inputClass}><option value="">All providers</option>{providers.data?.data.map((provider) => <option key={provider} value={provider}>{provider}</option>)}</select><Button secondary onClick={() => { setSearch(""); setProviderFilter(""); }}>Clear</Button></div>
      {files.isLoading ? <LoadingRows /> : files.error ? <ErrorNotice message={getApiErrorMessage(files.error)} /> : files.data?.data.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1240px] text-left text-sm"><thead className="text-[10px] uppercase tracking-wider text-slate-500"><tr><th className="pb-3">File</th><th className="pb-3">SEO metadata</th><th className="pb-3">Category</th><th className="pb-3">Provider</th><th className="pb-3">Size</th><th className="pb-3">Created</th><th className="pb-3">Migrate</th><th className="pb-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-white/8">{files.data.data.map((file) => <tr key={idOf(file)}><td className="py-4"><p className="font-bold">{file.label || file.asset.originalName}</p><p className="mt-1 max-w-xs truncate text-xs text-slate-500">{file.asset.storageKey}</p></td><td className="py-4"><p className="max-w-xs truncate text-xs font-bold text-slate-300">{file.asset.title || "No title"}</p><p className="mt-1 max-w-xs truncate text-xs text-slate-500">{file.asset.altText || "No alt text"}</p></td><td className="py-4"><StatusBadge value={file.category} /></td><td className="py-4"><StatusBadge value={file.asset.provider} /></td><td className="py-4 text-xs text-slate-400">{formatBytes(file.asset.size)}</td><td className="py-4 text-xs text-slate-500">{formatDate(file.createdAt)}</td><td className="py-4"><select aria-label={`Migrate ${file.asset.originalName}`} value={file.asset.provider} disabled={busy || (providers.data?.data.length ?? 0) < 2} onChange={(event) => void migrateFile(file, event.target.value)} className={`${inputClass} min-w-36`}>{providers.data?.data.map((provider) => <option key={provider} value={provider}>{provider}</option>)}</select></td><td className="py-4"><div className="flex justify-end gap-2"><Button secondary disabled={busy} onClick={() => beginEdit(file)}>SEO</Button>{file.asset.url ? <a href={file.asset.url} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center rounded-xl border border-white/10 px-4 py-2 text-sm font-black">Open</a> : null}<Button danger disabled={busy} onClick={() => void deleteFile(file)}>Delete</Button></div></td></tr>)}</tbody></table></div> : <Empty title="No managed files" description="Uploaded assets will appear here with permanent storage identity and SEO metadata." />}
