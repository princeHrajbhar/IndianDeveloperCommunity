...
import {
  ...
  useCommunicateWithLeadMutation,
...
export function LeadsPanel() {
  const [page,setPage]=useState(1),[search,setSearch]=useState(""),[status,setStatus]=useState<LeadStatus|"">(""),[fromDate,setFromDate]=useState(""),[toDate,setToDate]=useState(""),[selected,setSelected]=useState<string[]>([]),[assigneeSearch,setAssigneeSearch]=useState(""),[assignee,setAssignee]=useState(""),[exporting,setExporting]=useState(false),[notice,setNotice]=useState(""),[exportError,setExportError]=useState("");
  const list=useGetLeadsQuery({page,limit:25,...(search.trim()?{search:search.trim()}:{}),...(status?{status}:{}),...(fromDate?{fromDate}:{}),...(toDate?{toDate}:{})});const statistics=useGetLeadStatisticsQuery();const users=useGetUsersQuery({page:1,limit:100,...(assigneeSearch.trim()?{search:assigneeSearch.trim()}:{})});const[bulkAssign,bulkState]=useBulkAssignLeadsMutation();const stats=statistics.data?.data;const rows=list.data?.data??[];const all=rows.length>0&&rows.every(x=>selected.includes(x.id??x._id));
  async function assignSelected(){if(!selected.length)return;setExportError("");try{const r=await bulkAssign({ids:selected,assignedTo:assignee||null}).unwrap();setNotice(`${r.data.updated} leads assigned.`);setSelected([])}catch(e){setExportError(getApiErrorMessage(e))}}
  async function exportRows(){setExportError("");setExporting(true);try{await downloadAdminFile("/lead/admin/export","quantumfinix-leads.xlsx",{...(search.trim()?{search:search.trim()}:{}),...(status?{status}:{}),...(fromDate?{fromDate}:{}),...(toDate?{toDate}:{})})}catch(e){setExportError(getApiErrorMessage(e))}finally{setExporting(false)}}
  return <section><PageHeading eyebrow="Business development" title="Lead workspace" description="Search, filter by date, bulk assign owners, manage follow-ups and keep communication history on each lead." action={<><Button secondary disabled={exporting} onClick={()=>void exportRows()}>{exporting?"Exporting…":"Export Excel"}</Button><Link href="/dashboard/lead/add" className="rounded-xl bg-cyan-300 px-5 py-3 text-sm font-black text-slate-950">Create lead</Link></>}/>{exportError?<ErrorNotice message={exportError}/>:null}{notice?<SuccessNotice message={notice}/>:null}<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Total leads" value={stats?.total??"—"}/><Metric label="New today" value={stats?.newToday??"—"}/><Metric label="Unassigned" value={stats?.unassigned??"—"}/><Metric label="Overdue follow-ups" value={stats?.overdueFollowUps??"—"}/></div><div className="mt-6"><Panel><PanelTitle eyebrow="Enquiries" title="Lead pipeline"/><div className="grid gap-3 lg:grid-cols-5"><input value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}} placeholder="Search name, email, phone or purpose" className={inputClass}/><select value={status} onChange={e=>{setStatus(e.target.value as LeadStatus|"");setPage(1)}} className={inputClass}>{statuses.map(x=><option key={x||"all"} value={x}>{x||"All statuses"}</option>)}</select><input type="date" aria-label="From date" value={fromDate} onChange={e=>{setFromDate(e.target.value);setPage(1)}} className={inputClass}/><input type="date" aria-label="To date" value={toDate} onChange={e=>{setToDate(e.target.value);setPage(1)}} className={inputClass}/><Button secondary onClick={()=>{setSearch("");setStatus("");setFromDate("");setToDate("")}}>Clear</Button></div><div className="my-4 grid gap-3 rounded-2xl border border-white/8 bg-black/20 p-4 lg:grid-cols-[auto_1fr_1fr_auto]"><span className="self-center text-sm font-bold">{selected.length} selected</span><input value={assigneeSearch} onChange={e=>setAssigneeSearch(e.target.value)} placeholder="Search user for assignment" className={inputClass}/><select value={assignee} onChange={e=>setAssignee(e.target.value)} className={inputClass}><option value="">Unassigned</option>{users.data?.data.map(u=><option key={u.id??u._id} value={u.id??u._id}>{u.email}</option>)}</select><Button disabled={!selected.length||bulkState.isLoading} onClick={()=>void assignSelected()}>Bulk assign</Button></div>{list.isLoading?<LoadingRows/>:list.error?<ErrorNotice message={getApiErrorMessage(list.error)}/>:rows.length?<div className="overflow-x-auto"><table className="w-full min-w-[1000px] text-left text-sm"><thead className="text-[10px] uppercase tracking-wider text-slate-500"><tr><th className="pb-3"><input type="checkbox" checked={all} onChange={()=>setSelected(all?[]:rows.map(x=>x.id??x._id))}/></th><th className="pb-3">Lead</th><th className="pb-3">Purpose</th><th className="pb-3">Source</th><th className="pb-3">Priority</th><th className="pb-3">Status</th><th className="pb-3">Created</th><th className="pb-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-white/8">{rows.map(lead=>{const id=lead.id??lead._id;return <tr key={id}><td className="py-4"><input type="checkbox" checked={selected.includes(id)} onChange={()=>setSelected(a=>a.includes(id)?a.filter(x=>x!==id):[...a,id])}/></td><td className="py-4"><Link href={`/dashboard/lead/${id}`} className="font-bold hover:text-cyan-300">{lead.name}</Link><p className="mt-1 text-xs text-slate-500">{lead.email} · {lead.phone}</p></td><td className="max-w-xs py-4"><p className="line-clamp-2 text-slate-400">{lead.purpose}</p></td><td className="py-4 text-slate-400">{lead.source}</td><td className="py-4"><StatusBadge value={lead.priority}/></td><td className="py-4"><StatusBadge value={lead.status}/></td><td className="py-4 text-xs text-slate-500">{formatDate(lead.createdAt)}</td><td className="py-4 text-right"><Link href={`/dashboard/lead/${id}`} className="rounded-xl border border-cyan-300/20 px-4 py-2.5 text-xs font-bold text-cyan-200">Manage</Link></td></tr>})}</tbody></table></div>:<Empty title="No leads found" description="New contact, consultation and administrator-created leads will appear here."/>}<Pagination page={page} totalPages={list.data?.pagination.totalPages??1} onPageChange={setPage}/></Panel></div></section>;
}
...
export function LeadAdminDetail({ leadId }: { leadId: string }) {
  ...
  const users = useGetUsersQuery({ page: 1, limit: 100, ...(userSearch.trim() ? { search: userSearch.trim() } : {}) });
  ...
  const [communicate, communicateState] = useCommunicateWithLeadMutation();
  const [notice, setNotice] = useState("");
  const [localError, setLocalError] = useState("");
  const [note, setNote] = useState("");
  const [comm, setComm] = useState({ kind: "email" as "email"|"followup", subject: "", message: "", nextFollowUpAt: "" });
  ...
  async function sendCommunication() { if (!comm.message.trim()) return; setLocalError(""); try { await communicate({ id: leadId, kind: comm.kind, ...(comm.subject.trim()?{subject:comm.subject.trim()}:{}), message:comm.message.trim(), ...(comm.nextFollowUpAt?{nextFollowUpAt:new Date(comm.nextFollowUpAt).toISOString()}:{}) }).unwrap(); setComm({kind:"email",subject:"",message:"",nextFollowUpAt:""}); setNotice(comm.kind === "email" ? "Email sent and conversation logged." : "Follow-up logged."); } catch(caught){setLocalError(getApiErrorMessage(caught));} }
  ...
  const busy = updateState.isLoading || noteState.isLoading || deleteState.isLoading || communicateState.isLoading;

  return <section><PageHeading eyebrow="Lead record" title={selected.name} description={`${selected.email} · ${selected.phone}`} action={<Link href="/dashboard/lead" className="rounded-xl border border-white/10 px-4 py-2.5 text-sm font-bold">Back</Link>} />{localError ? <ErrorNotice message={localError} /> : null}{notice ? <SuccessNotice message={notice} /> : null}<div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_390px]"><div className="space-y-6"><Panel><div className="grid gap-3 sm:grid-cols-2"><Info label="Email" value={selected.email} /><Info label="Phone" value={selected.phone} /><Info label="Location" value={selected.location} /><Info label="Contact preference" value={selected.contactPreference} /><Info label="Page" value={selected.pagePath} /><Info label="Created" value={formatDate(selected.createdAt)} /></div><div className="mt-5 rounded-2xl border border-white/8 bg-black/20 p-5"><p className="text-xs font-bold uppercase tracking-wider text-cyan-300">Purpose</p><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-slate-300">{selected.purpose}</p></div></Panel><Panel><PanelTitle eyebrow="History" title="Internal notes" /><div className="space-y-3">{selected.notes?.length ? selected.notes.map((item, index) => <div key={item._id ?? index} className="rounded-2xl border border-white/8 bg-black/20 p-4"><p className="text-sm leading-6 text-slate-300">{item.text}</p><p className="mt-2 text-xs text-slate-600">{formatDate(item.createdAt)}</p></div>) : <p className="text-sm text-slate-500">No notes yet.</p>}</div><div className="mt-4 flex gap-3"><textarea rows={3} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Add an internal note" className={textareaClass} /><Button disabled={busy || !note.trim()} onClick={() => void saveNote()}>Add note</Button></div></Panel><Panel><PanelTitle eyebrow="Communication" title="Email & follow-up history" /><div className="grid gap-3 md:grid-cols-2"><Field label="Type"><select value={comm.kind} onChange={e=>setComm({...comm,kind:e.target.value as "email"|"followup"})} className={inputClass}><option value="email">Email</option><option value="followup">Follow-up log</option></select></Field><Field label="Next follow-up"><input type="datetime-local" value={comm.nextFollowUpAt} onChange={e=>setComm({...comm,nextFollowUpAt:e.target.value})} className={inputClass}/></Field>{comm.kind==="email"?<div className="md:col-span-2"><Field label="Subject"><input value={comm.subject} onChange={e=>setComm({...comm,subject:e.target.value})} className={inputClass}/></Field></div>:null}<div className="md:col-span-2"><Field label="Message / follow-up note"><textarea rows={4} value={comm.message} onChange={e=>setComm({...comm,message:e.target.value})} className={textareaClass}/></Field></div><div className="md:col-span-2"><Button disabled={busy||!comm.message.trim()} onClick={()=>void sendCommunication()}>{comm.kind==="email"?"Send email & log":"Log follow-up"}</Button></div></div><div className="mt-6 space-y-3">{selected.communications?.length? [...selected.communications].reverse().map((c,i)=><div key={c._id??i} className="rounded-2xl border border-white/8 bg-black/20 p-4"><div className="flex items-center justify-between gap-3"><StatusBadge value={c.kind}/><span className="text-xs text-slate-600">{formatDate(c.sentAt)}</span></div>{c.subject?<p className="mt-3 font-bold">{c.subject}</p>:null}<p className="mt-2 whitespace-pre-wrap text-sm text-slate-300">{c.message}</p>{c.nextFollowUpAt?<p className="mt-2 text-xs text-cyan-300">Follow-up: {formatDate(c.nextFollowUpAt)}</p>:null}</div>):<p className="text-sm text-slate-500">No communication history yet.</p>}</div></Panel></div><aside><Panel><PanelTitle eyebrow="Pipeline" title="Manage lead" /><div className="space-y-4"><Select label="Status" value={form.status} values={statuses.filter(Boolean) as LeadStatus[]} set={(value) => setForm({ ...form, status: value as LeadStatus })} /><Select label="Priority" value={form.priority} values={priorities} set={(value) => setForm({ ...form, priority: value as LeadPriority })} /><Select label="Source" value={form.source} values={sources} set={(value) => setForm({ ...form, source: value as LeadSource })} /><Field label="Search assigned owner"><input value={userSearch} onChange={e=>setUserSearch(e.target.value)} placeholder="Search user email" className={inputClass}/></Field><Field label="Assigned owner"><select value={form.assignedTo} onChange={(event) => setForm({ ...form, assignedTo: event.target.value })} className={inputClass}><option value="">Unassigned</option>{users.data?.data.map((user) => <option key={user.id ?? user._id} value={user.id ?? user._id}>{user.email}</option>)}</select></Field><Text label="Next follow-up" value={form.followUp} set={(value) => setForm({ ...form, followUp: value })} type="datetime-local" />{form.status === "Lost" ? <Area label="Lost reason" value={form.lostReason} set={(value) => setForm({ ...form, lostReason: value })} required /> : null}<Text label="Tags" value={form.tags} set={(value) => setForm({ ...form, tags: value })} /><Area label="Internal summary" value={form.summary} set={(value) => setForm({ ...form, summary: value })} /><Button disabled={busy} onClick={() => void saveLead()}>{updateState.isLoading ? "Saving…" : "Save lead"}</Button><Button danger disabled={busy} onClick={() => void remove()}>Delete lead</Button></div></Panel></aside></div></section>;
}
