...
import { BarChart3, Bell, Download, Flame, LayoutDashboard, PanelLeftClose, Search, ShieldCheck, UserCheck, UserRoundSearch, Users, X } from "lucide-react";
...
export function LeadManagementShell({ children }: { children: ReactNode }) {
  ...
  const sidebar = <aside className={`${compact ? "lg:w-[88px]" : "lg:w-[292px]"} qf-surface flex h-full w-[292px] flex-col border-r transition-[width] duration-200`}>
    ...
    <nav className="qf-sidebar-scroll min-h-0 flex-1 overflow-y-auto px-3 py-4">
      ...
      <div className="space-y-1">{visible.map((item) => { const baseHref = item.href.split("?")[0]; const itemView = new URLSearchParams(item.href.split("?")[1] ?? "").get("view"); const currentView = searchParams.get("view"); const active = item.exact ? pathname === baseHref : itemView ? pathname === baseHref && currentView === itemView : pathname === baseHref && !currentView || pathname.startsWith(`${baseHref}/`); const Icon = item.icon; return <Link key={item.href} href={item.href} title={compact ? item.label : undefined} className={`flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-bold transition ${active ? "bg-blue-600 text-white" : "qf-text-secondary hover:bg-[var(--qf-surface-muted)] hover:text-[var(--qf-text)]"} ${compact ? "justify-center px-2" : ""}`}><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg ${active ? "bg-white/12" : "bg-[var(--qf-surface-muted)] text-blue-600"}`}><Icon className="h-4 w-4"/></span>{!compact ? <span className="truncate">{item.label}</span> : null}</Link>; })}</div>
    </nav>
