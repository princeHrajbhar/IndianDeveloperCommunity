...
const BlogListingPage = () => {
  ...
  // Truncate description and add "Read more" link
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            ...
            className="mt-4 px-6 py-2 bg-[#22d3ee] text-white rounded-lg hover:bg-[#0ea5e9] hover:shadow-lg hover:shadow-[#22d3ee]/25 transition-all"
          >
            Retry
          </button>
        </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3.5 bg-white text-gray-900 placeholder-gray-500 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#22d3ee] transition-all shadow-sm border border-gray-200 text-sm"
              />
              {searchTerm && (
                <button
                  type="button"
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 mb-6">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-full transition-all ${
                selectedCategory === ''
                  ? 'bg-[#22d3ee] text-white shadow-sm shadow-[#22d3ee]/25'
        ...
        {paginatedBlogs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginatedBlogs.map((blog) => {
              const truncatedDesc = truncateDescription(blog.description, 100);
              const isTruncated = blog.description?.length > 100;
              
              return (
                <Link
                  key={blog._id}
                  href={`/blog/${blog.slug}`}
                  className="group bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden hover:border-[#22d3ee]/30"
                >
                  ...
                  <div className="relative w-full pt-[56.25%] overflow-hidden">
                    {blog.banner?.url ? (
                      <Image
                        src={blog.banner.url}
                        alt={blog.banner.altText || blog.banner.title || blog.title}
                        title={blog.banner.title || blog.title}
                        fill
                  ...
                  <div className="p-5">
                    <h2 className="text-lg font-semibold text-gray-900 group-hover:text-[#22d3ee] transition-colors mb-2 line-clamp-2">
                      {blog.title}
                    </h2>
                    ...
                    <p className="text-sm text-gray-500 mb-2">
                      {truncatedDesc}
                      {isTruncated && (
                        <span className="text-[#22d3ee] font-medium ml-1 inline-block group-hover:text-[#6cb84d] transition-colors">
                          Read more
                        </span>
                      )}
                    </p>
          ...
          <div className="flex items-center justify-between mt-8 pt-4 border-t border-gray-200 flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 border border-gray-300 rounded-lg hover:bg-[#22d3ee] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors bg-white"
              >
