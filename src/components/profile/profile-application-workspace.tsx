...
export function ProfileApplicationDetail({ applicationId }: { applicationId: string }) {
  ...
  async function chooseInterviewSlot(interview: HRInterview, slot: string) {
    if (!window.confirm(`Confirm interview for ${formatInterviewDateTime(slot, interview.availability?.timezone || "Asia/Kolkata")}?`)) return;
    setError(""); setInterviewNotice("");
    try {
      await selectInterviewSlot({ id: interview.id, slot }).unwrap();
      setInterviewNotice("Interview time confirmed. Your meeting details are now available here.");
...
function CandidateInterviewCard({ interview, busy, choose }: { interview: HRInterview; busy: boolean; choose: (slot: string) => void }) {
  ...
  const timezone = interview.availability?.timezone || "Asia/Kolkata";
  const grouped = groupInterviewSlots(slots, timezone);
  const dates = Object.keys(grouped);
  const [selectedDate, setSelectedDate] = useState("");
  const activeDate = selectedDate && grouped[selectedDate] ? selectedDate : dates[0] || "";
  return <div className="rounded-2xl border border-white/10 bg-black/20 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-cyan-300">{interview.roundName}</p><h3 className="mt-2 text-lg font-black">{interview.status === "awaiting-candidate" ? "Choose your interview time" : interview.status === "scheduled" ? "Interview confirmed" : interview.status === "completed" ? "Interview completed" : "Interview update"}</h3></div><StatusBadge value={interview.status}/></div>{interview.status === "awaiting-candidate" ? <><p className="mt-4 text-sm leading-6 text-slate-400">Choose a date first, then select one currently available time. Times are shown in {timezone}; unavailable or already-booked slots are removed automatically.</p>{slots.length ? <><div className="mt-4 flex snap-x gap-2 overflow-x-auto pb-2">{dates.map((date) => <button type="button" key={date} onClick={() => setSelectedDate(date)} className={`min-w-[112px] snap-start rounded-xl border px-3 py-2.5 text-left transition ${activeDate === date ? "border-cyan-300 bg-cyan-300 text-slate-950" : "border-white/10 bg-white/[0.03] text-slate-200 hover:border-cyan-300/40"}`}><span className="block text-[10px] font-black uppercase tracking-wider opacity-70">{formatInterviewWeekday(date)}</span><span className="mt-1 block text-sm font-black">{formatInterviewDate(date)}</span><span className="mt-1 block text-[10px] font-bold opacity-70">{grouped[date].length} slot{grouped[date].length === 1 ? "" : "s"}</span></button>)}</div>{activeDate ? <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">{grouped[activeDate].map((slot) => <button type="button" disabled={busy} onClick={() => choose(slot)} key={slot} className="rounded-xl border border-cyan-300/20 bg-cyan-300/[0.06] px-3 py-3 text-center text-xs font-black text-cyan-100 transition hover:border-cyan-300/50 hover:bg-cyan-300/[0.1] disabled:opacity-50">{formatInterviewTime(slot, timezone)}</button>)}</div> : null}</> : <p className="mt-4 rounded-xl border border-amber-300/15 bg-amber-300/[0.06] px-4 py-3 text-sm text-amber-100">No open slots are currently available. HR may update the availability window.</p>}</> : <div className="mt-4 grid gap-3 sm:grid-cols-2"><Stat label="Date & time" value={formatInterviewDateTime(interview.scheduledAt, timezone)} /><Stat label="Mode" value={interview.mode} />{interview.location ? <Stat label="Location" value={interview.location}/> : null}{interview.meetingUrl ? <a href={interview.meetingUrl} target="_blank" rel="noreferrer" className="rounded-2xl border border-cyan-300/20 bg-cyan-300 px-4 py-4 text-center text-sm font-black text-slate-950">Open interview meeting</a> : <Stat label="Meeting link" value="HR will add it before the interview"/>}</div>}</div>;
}
function groupInterviewSlots(slots: string[], timezone: string) { const rows: Record<string, string[]> = {}; for (const slot of slots) { const date = new Date(slot); const parts = new Intl.DateTimeFormat("en-CA", { timeZone: timezone, year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(date); const get = (type: string) => parts.find((part) => part.type === type)?.value || ""; const key = `${get("year")}-${get("month")}-${get("day")}`; (rows[key] ||= []).push(slot); } for (const values of Object.values(rows)) values.sort((a, b) => new Date(a).getTime() - new Date(b).getTime()); return Object.fromEntries(Object.entries(rows).sort(([a], [b]) => a.localeCompare(b))); }
function formatInterviewWeekday(date: string) { return new Date(`${date}T12:00:00Z`).toLocaleDateString("en-IN", { weekday: "short" }); }
function formatInterviewDate(date: string) { return new Date(`${date}T12:00:00Z`).toLocaleDateString("en-IN", { day: "numeric", month: "short" }); }
function formatInterviewTime(value: string, timezone: string) { const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleTimeString("en-IN", { timeZone: timezone, hour: "numeric", minute: "2-digit" }); }

function formatInterviewDateTime(value?: string, timezone = "Asia/Kolkata") { if (!value) return "—"; const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleString("en-IN", { timeZone: timezone, dateStyle: "medium", timeStyle: "short" }); }
