...
export function HRManagementShell({ children }: { children: ReactNode }) {
  ...
  const sidebar = (
    <aside className={`${compact ? "lg:w-[88px]" : "lg:w-[292px]"} qf-surface flex h-full w-[292px] flex-col border-r transition-[width] duration-200`}>
      <div className="qf-border flex h-20 items-center justify-between border-b px-4">
        <div className="flex min-w-0 items-center gap-3">
          <DashboardBrandLogo href="/" label="QuantumFinix home" className={`${compact ? "h-10 w-12" : "h-10 w-[152px]"} object-contain object-left`} />
          {!compact ? <span className="qf-muted border-l border-slate-200 pl-3 text-[10px] font-black uppercase tracking-[.16em]">HRMS</span> : null}
        </div>
        <button onClick={toggleCompact} className="qf-icon-button hidden h-9 w-9 lg:grid" aria-label="Toggle HR sidebar">
          <PanelLeftClose className={`h-4 w-4 ${compact ? "rotate-180" : ""}`} />
        </button>
      </div>
      ...
      <nav className="qf-sidebar-scroll min-h-0 flex-1 overflow-y-auto px-3 py-4">
        {visibleGroups.map((group) => (
          <div key={group.label} className="mb-5">
            {!compact ? <p className="qf-muted mb-2 px-3 text-[10px] font-black uppercase tracking-[.16em]">{group.label}</p> : null}
              ...
              {group.items.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-bold transition ${active ? "bg-blue-600 text-white" : "qf-text-secondary hover:bg-[var(--qf-surface-muted)] hover:text-[var(--qf-text)]"} ${compact ? "justify-center px-2" : ""}`}
                  >
    ...
    <div className="qf-app-shell min-h-screen">
      ...
      {mobile ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="relative h-full">{sidebar}<button onClick={() => setMobile(false)} className="qf-icon-button absolute right-3 top-3"><X className="h-4 w-4" /></button></div>
        </div>
      ) : null}
      ...
      <div className={`${compact ? "lg:pl-[88px]" : "lg:pl-[292px]"} transition-[padding] duration-200`}>
        <header className="qf-surface sticky top-0 z-30 flex h-20 items-center gap-3 border-b px-4 backdrop-blur md:px-6">
          ...
          <div className="relative ml-auto max-w-xl flex-1 md:ml-5">
            <Search className="qf-muted pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2" />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search HR modules…" className="qf-input h-11 w-full rounded-xl pl-10 pr-4 text-sm" />
            {query.trim() ? (
        ...
        <main className="mx-auto max-w-[1720px] p-4 md:p-6 xl:p-8">{hasPageAccess ? children : <div className="grid min-h-[55vh] place-items-center"><div className="qf-surface qf-shadow max-w-lg rounded-2xl border p-8 text-center"><ShieldCheck className="mx-auto h-9 w-9 text-[var(--qf-primary-text)]"/><h1 className="qf-text mt-4 text-2xl font-black">HR module access required</h1><p className="qf-muted mt-3 text-sm leading-6">Your role does not include this HRMS module. Ask an authorized administrator to update the required permission.</p><Link href="/hr-management" className="qf-primary-button mt-6 inline-flex rounded-xl px-5 py-3 text-sm font-bold">Return to HR Overview</Link></div></div>}</main>
      </div>
    </div>
  );
}
