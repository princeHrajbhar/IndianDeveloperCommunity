 ...
 * adapted for QuantumFinix. Used for blog content and course CMS content.
 *
 * Features:
 ...
 *  - Image insert via QuantumFinix managed-file upload or by URL
...
// Resolve the QuantumFinix backend URL used by the shared RTK Query layer.
...
export default function RichTextEditor({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filenameWithoutExtension = file.name.replace(/\.[^.]+$/, "").replace(/[-_]+/g, " ").trim();
      const meaningfulTitle = filenameWithoutExtension || "Content image";
      formData.append("file", file);
      formData.append("category", "content-images");
      formData.append("title", meaningfulTitle);
      formData.append("altText", meaningfulTitle);
      formData.append("label", meaningfulTitle);
      formData.append("filename", meaningfulTitle);

      const token =
        typeof window !== "undefined" ? localStorage.getItem("accessToken") : null;

      const res = await fetch(`${resolveApiBaseUrl()}/files/admin`, {
        method: "POST",
        credentials: "include",
      ...
      const uploadedUrl = json?.data?.asset?.url || json?.data?.url;
