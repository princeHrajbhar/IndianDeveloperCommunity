...
const getCourseLevelLabel = (value: unknown): string => {
  ...
  return labels[normalizeCourseLevel(value)];
};
...
export default function CoursesPage() {
  ...
  const filteredCourses = useMemo(() => {
    ...
    if (searchLower) {
      filtered = filtered.filter((course) => {
        ...
        const level = getCourseLevelLabel(course.level).toLowerCase();
        const keywordMatches = course.keywords?.some((keyword) =>
    ...
    if (selectedLevel !== 'all') {
      filtered = filtered.filter(
        (course) => normalizeCourseLevel(course.level) === selectedLevel,
      );
    }
  ...
  const formatPrice = (price?: number, currency = 'INR') => {
    ...
    return `${symbols[normalizedCurrency] || normalizedCurrency} ${normalizedPrice.toFixed(2)}`;
  };

  // Handle view course - open in new tab
  ...
  // Handle edit course - redirect to dashboard edit page
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
        ...
        {activeFilterCount > 0 && (
          <div className="flex flex-wrap items-center gap-2 pt-2">
            ...
            {selectedLevel !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-teal-50 text-teal-700 text-xs rounded-full">
                Level: {getCourseLevelLabel(selectedLevel)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div>
                ...
                >
                  ...
                  <option value="all-levels">All-level Courses</option>
        ...
        {showLoading && courses.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-500 mt-1">
              ...
              ) : selectedStatus !== 'all' || selectedCategory !== 'all' || selectedLevel !== 'all' ? (
                'Try adjusting your filters to find what you\'re looking for'
              ) : (
                'Create your first course to get started'
              )}
            </p>
            {(searchQuery.trim() || selectedStatus !== 'all' || selectedCategory !== 'all' || selectedLevel !== 'all') && (
              <button
                onClick={clearAllFilters}
            ...
            {!searchQuery && selectedStatus === 'all' && selectedCategory === 'all' && selectedLevel === 'all' && (
              <button
                onClick={() => router.push('/dashboard/courses/add')}
                className="mt-4 px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors"
              >
              ...
              <table className="w-full min-w-[640px]">
                <thead className="bg-gray-50/80 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-4 py-3.5 hidden xl:table-cell min-w-[110px]">
                      Level
                    </th>
                    ...
                    <tr key={course._id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-4 py-3.5 hidden lg:table-cell">
                        <div className="flex flex-col">
                          ...
                          {typeof course.discountedPrice === 'number' && typeof course.price === 'number' && course.discountedPrice > 0 && course.discountedPrice < course.price && (
                      ...
                      <td className="px-4 py-3.5 hidden xl:table-cell">
                        <span className="inline-flex items-center rounded-full bg-sky-50 px-2.5 py-1.5 text-xs text-sky-700 whitespace-nowrap">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center justify-end gap-1">
                          <button 
                            ...
                            title="View Course (opens in new tab)"
                          >
