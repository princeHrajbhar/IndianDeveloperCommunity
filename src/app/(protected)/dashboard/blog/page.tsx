...
export default function BlogPage() {
  ...
  // Build query parameters for server-side pagination, filtering, and search.
  ...
  // Debounce search so the API is not called for every keystroke.
  useEffect(() => {
  ...
  // Refetch when pagination, filters, sorting, or server search changes.
  useEffect(() => {
    refetchBlogs();
    if (isInitialLoad) {
  ...
  // Handle view blog - opens in new tab with slug
  const handleViewBlog = (post: any) => {
    // Use the blog slug if available, otherwise fallback to ID
  ...
  // Search is performed by the backend across the complete blog collection.
          ...
          <p className="text-sm text-gray-500 mt-1">
            {showLoading ? 'Loading...' : (
              <>
                {searchQuery.trim() ? (
                  <span>
                    Found <span className="font-medium text-gray-700">{pagination?.total || 0}</span> matching posts
                  </span>
                ) : (
          ...
          <>
                          ...
                          <div className="flex items-center gap-3">
                            {post.banner?.url ? (
                              <div className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100">
                                <img 
                                  src={post.banner.url} 
                                  alt={post.banner.altText || post.banner.title || post.title}
                                  title={post.banner.title || post.title}
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex flex-wrap gap-1 mt-1">
                                {post.keyword?.slice(0, 2).map((tag: string, i: number) => (
            ...
            {searchQuery.trim() && (
              <div className="px-4 py-3 border-t border-gray-200 bg-gray-50/50">
                <p className="text-sm text-gray-500">
                  Showing <span className="font-medium text-gray-700">{displayBlogs.length}</span> of{' '}
                  <span className="font-medium text-gray-700">{pagination?.total || 0}</span> matching posts
                </p>
              </div>
            )}
          </>
