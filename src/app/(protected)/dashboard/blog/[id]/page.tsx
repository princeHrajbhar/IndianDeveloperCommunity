...
const getMediaSeo = (file?: UploadedFile | null): MediaSeoValue => ({
  title: file?.title || '',
...
export default function EditBlogPage() {
  ...
  const [bannerSeo, setBannerSeo] = useState<MediaSeoValue>(getMediaSeo());
  ...
  const { data: blogData, isLoading: fetching } = useGetBlogById(id);
  ...
  useEffect(() => {
    if (blogData?.data) {
      ...
      setFormData({
        ...
        postedBy: blog.postedBy || authorName,
      ...
      if (blog.banner?.url && blog.banner?.storageKey) {
        ...
        setBannerSeo(getMediaSeo(blog.banner));
        setBannerPreview(blog.banner.url);
        setRemoveBanner(false);
      } else {
        setExistingBanner(null);
        setBannerSeo(getMediaSeo());
  ...
  const handleBannerChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setBannerSeo(getMediaSeo(existingBanner));
  ...
  const updateExistingResourceSeo = (index: number, next: MediaSeoValue) => {
    setExistingResources((previous) =>
      previous.map((resource, resourceIndex) =>
        resourceIndex === index ? { ...resource, ...next } : resource,
      ),
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!removeBanner && (bannerFile || existingBanner)) {
              ...
              >
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    {!removeBanner && (bannerFile || existingBanner) && (
                      <MediaSeoFields
                        value={bannerSeo}
                    ...
                    {existingResources.length > 0 && (
                      <div className="mt-4 space-y-2">
                        ...
                        {existingResources.map((resource, index) => (
                          <div key={resource.storageKey || `existing-${index}`} className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                            ...
                            <MediaSeoFields
                              value={getMediaSeo(resource)}
                              onChange={(next) => updateExistingResourceSeo(index, next)}
                              accent="blue"
                              compact
                            />
                          </div>
                    ...
                    {resourceFiles.length > 0 && (
                      <div className="mt-4 space-y-2">
                        ...
                        {resourceFiles.map((file, index) => (
                          <div key={`new-${index}`} className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                            <div className="flex items-center justify-between gap-3">
                              <div className="flex min-w-0 items-center gap-3">
                                <div className="flex-shrink-0 rounded-lg bg-blue-100 p-2">
                                  <FileText className="w-4 h-4 text-blue-600" />
